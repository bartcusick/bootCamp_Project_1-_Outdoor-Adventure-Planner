  ...
  geocoder.geocode(
    ...
    (results, status) => {
      if (status === "OK") {
        ...
        var trailName = $("<div>")
          .text("Please wait while we search for trails in " + address)
          .addClass("py-3");
...
var getTrails = function () {
  locale(function (lat, lon) {
    ...
    var trailUrl =
      "https://trailapi-trailapi.p.rapidapi.com/trails/explore/?&per_page= " +
      results +
      "&radius=" +
      radius +
      "&lat=" +
    ...
    .then(function (response) {
      ...
      $(".placeholder").html(
        "<div class=has-text-centered id=location>Select Trail to View Info</div><img src='./assets/birdBikeMap.png' alt='Bird with Map'> "
      );
...
var trailInfo = function (trailID) {
  ...
  $.ajax({
    ...
    results.forEach((data) => {
      ...
      $(".more-info").html(
        "<br><span class=has-text-weight-bold>Detailed Info:</span> <a href='" +
          data.url +
          "' target=_blank>Click to find out more about this trail.</a>"
      );
